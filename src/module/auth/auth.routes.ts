...
import { RegisterDto, LoginDto, ForgotPasswordDto, NewPasswordDto } from "../dto/auth.dto";
...
router.get("/logout", validateUserMiddleware, customerController.logoutCustomer);
router.get("/refresh", customerController.refreshCustomer);
router.post("/forgot-password", validate(ForgotPasswordDto), customerController.forgotPassword);
router.post("/new-password/:token", validate(NewPasswordDto), customerController.newPassword);
